...
> 生成日期：2026-09-05
> 定位：面向企业内部的仓储管理系统（WMS），聚焦库存账实一致、并发正确性、多仓数据隔离与权限模型。
> 详细背景见 [PROJECT_PLAN.md](./PROJECT_PLAN.md)。

---

## 一、总原则

1. **深度 > 广度**：把一个领域做透、能说清「为什么这么设计」，胜过十项浅尝辄止的功能。
2. **可证明 > 自述**：凡声称的设计，都要有测试或代码证据背书（单测 / 架构测试 / 唯一约束），不能只写在注释或 README 里。
3. **关键路径优先**：资源集中投入库存一致性这一核心链路，而非平均铺开。
4. **不堆中间件**：不为用而用 Redis/MQ/微服务；只补能说清「为什么」的东西。
...
**结论**：库存一致性这一核心链路已具备主体，后续聚焦「补测试证据 + 补功能缺口 + 补运行验证」。

---

## 三、迭代项（按优先级，均已完成，见执行记录）

### P0-1 幂等入账（防重复入账）—— 库存一致性核心链路的最后一块缺口
...
> 2026-09-05 调整：原「不做 Redis/MQ/前端」因实际需求变更而纳入——
> **Redis** 补「无状态 JWT 无法主动失效」的缺口（登出黑名单）；**RabbitMQ** 做「出入库完成领域事件」以解耦 ERP/WCS 通知（`AFTER_COMMIT` 投递）；**Vue3 前端** 提供仓储操作可视化界面，设计走 Apple 克制风格。三者均服务于具体业务场景，非「为用而用」，故从「不做」划出。
